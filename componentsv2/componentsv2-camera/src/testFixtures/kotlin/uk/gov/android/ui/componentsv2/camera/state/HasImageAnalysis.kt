...
internal class HasImageAnalysis(private val matcher: Matcher<ImageAnalysis>) :
    TypeSafeMatcher<ImageAnalyzer.State>() {
    override fun matchesSafely(item: ImageAnalyzer.State?): Boolean =
        matcher.matches(item?.imageAnalysis?.value)
