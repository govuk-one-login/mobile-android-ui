...
internal class HasPreview(private val matcher: Matcher<Preview>) :
    TypeSafeMatcher<ImagePreviewer.State>() {
    override fun matchesSafely(item: ImagePreviewer.State?): Boolean =
        matcher.matches(item?.preview?.value)
